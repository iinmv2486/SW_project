...
int calculateLedBrightness(float distance) {
  if (distance <= _DIST_MIN || distance >= _DIST_MAX) {
    return ; // LED off for distances <= 100mm or >= 300mm
  }
  
  // When distance = 200mm, brightness should be maximum i.e., 255
  // As distances deviate from 200mm (either towards 100mm or 300mm), brightness should decrease

  float deviation = abs(distance - 200); // How far is the distance from 200mm
  int brightness = (int)(2.55 * deviation) - 255;  // Map the deviation to a brightness value reduction
