...
void adjustServoPosition(float distance) {
  int servoPosition;
  if (distance < 180.0) {
    servoPosition = _DUTY_MIN;
  } else if (distance <= 360.0) {
    servoPosition = map(distance, 180.0, 360.0, _DUTY_MIN, _DUTY_MAX);
  } else {
    servoPosition = _DUTY_MAX;
  }
